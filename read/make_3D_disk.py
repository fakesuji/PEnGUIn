...
dat0 = rd.load_3D_data('/mnt/penguin/fung/p2/',xmax,ymax,zmax,label2,0)
...
def make_2D_density(xmax,ymax,z,den):
	...
	for i in range(xmax):
		for j in range(ymax):
			...
			den2D[j,i] /= np.sqrt(np.pi/2.0)*0.3
	return den2D

#den2D1 = make_2D_density(xmax,ymax,dat1[3],dat1[4])
den2D0 = make_2D_density(xmax,ymax,dat0[3],dat0[4])
...
plt.figure(4)
plt.pcolormesh(dat2[1],dat2[2],dat2[4][-1,:,:]-dat0[4][-1,:,:])
plt.colorbar()

plt.figure(5)
plt.pcolormesh(dat2[1],dat2[2],den2D2-den2D0)
plt.colorbar()
