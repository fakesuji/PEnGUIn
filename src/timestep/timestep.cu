...
double global_dt(Grid* dev, double dt)
{
	...
	for (int n=0; n<ndev; n++)
	{
		...
		cudaMalloc( (void**)&dt_lv1, lv1_size*sizeof(double) );
		get_dt_lv1<<< lv1_size, std_thd, std_thd*sizeof(double), dev[n].stream >>>(dt_lv1, &dev[n].xa[dev[n].xbgn], dev[n].ya, dev[n].za, nx, ny, nz, dev[n].C, dev[n].orb_rot);
		get_dt_lv2<<< 1, lv1_size, lv1_size*sizeof(double), dev[n].stream >>>(d_dt, dt_lv1);
		cudaMemcpy( &h_dt[n], d_dt, sizeof(double), cudaMemcpyDeviceToHost );
		cudaFree(d_dt);
		cudaFree(dt_lv1);
	}
